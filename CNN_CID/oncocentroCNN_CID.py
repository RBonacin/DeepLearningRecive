...
def create_CNN_model():
    ...
    model.add(layers.Conv1D(filters=230, kernel_size=3, padding='same', activation='relu'))
    model.add(layers.MaxPooling1D(pool_size=2))
    model.add(layers.Dropout(0.5))
    model.add(layers.Flatten())
    model.add(layers.Dense(22, activation="relu"))    # Tested from 12 to 44
...
def init():
    ...
    df = pd.read_csv(file)
    with open('outputALLCIDCNN_TOPO.txt', 'w') as log:
        log.close()
    return df
...
def StratifiedCrossCID(filtered_df,topo,size):
    ...
    for train_index,test_index in StratifiedKFold(n_split,random_state=25, shuffle=True).split(x,y):
      ...
      sauc = sauc + auc
      print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
      with open('outputALLCIDCNN_TOPO.txt', 'a') as log:
          log.write(str(topo))
          log.write(',')
          log.write(str(f1))
          log.write(',')
...
for topo in df['topo'].unique():   # For Each ICD
    ...
    if (size>200 and i > 65) :  # Ignore ICD with less than 200 instances
